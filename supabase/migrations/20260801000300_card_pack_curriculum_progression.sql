...
alter table public.card_pack_chapters
  ...
  add column prerequisite_chapter_ids jsonb not null default '[]'::jsonb
    check (jsonb_typeof(prerequisite_chapter_ids) = 'array');

create function public.publish_card_pack_v3(
  p_manifest jsonb,
  p_chapters jsonb,
...
begin
  ...
  if exists (
    ...
    where jsonb_typeof(chapter.value->'learningObjectives') is distinct from 'array'
      ...
      or (chapter.value->>'chapterId')::integer <> (chapter.value->>'position')::integer
  ) then
    raise exception 'Progressive Card Pack Chapter metadata is invalid';
  end if;
  if exists (
    select 1
    from jsonb_array_elements(p_chapters) as chapter(value)
  ...
  insert into public.card_pack_chapters (
    ...
    card_count, learning_objectives, prerequisite_chapter_ids
  ) select
  ...
  insert into public.card_pack_cards (
    ...
    '0x' || md5(card.value::text) || md5('MINDMARK_PACK_CARD_V3:' || card.value::text),
...
revoke execute on function public.publish_card_pack_v3(jsonb, jsonb, text, text) from public;
revoke execute on function public.get_published_card_pack_v1(uuid, text) from public;
do $$
begin
  if exists (select 1 from pg_roles where rolname = 'service_role') then
    grant execute on function public.publish_card_pack_v3(jsonb, jsonb, text, text) to service_role;
