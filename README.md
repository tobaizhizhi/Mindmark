...
Mindmark 将书籍、论文、课程讲义等长资料整理为有章节顺序、带原文依据的知识卡，并通过主动回忆与 FSRS 复习调度算法帮助学习者持续复习。对于 AI 生成部分，Monad 负责登记 Project、锁定 Sponsor 预算、记录每个 Work Unit 的结果承诺并结算 Worker Reward，Moss 在 Treasury 签名前审阅资金操作。一个上传型项目会产生 Project 登记、预算锁定、多个 Work Unit 承诺、多个 Chapter 完成记录、Project 最终确认和逐项 Reward 释放等多笔交易；Monad 的低成本、高吞吐和较快确认使这种细粒度链上记录与结算具备可行性。

**一句话简介**：Mindmark 用 AI 将长资料整理成带原文依据的知识卡片，通过主动回忆与 FSRS 复习调度算法帮助学习者复习，并利用 Monad 为每项 AI 生成任务锁定预算、登记质量通过后的结果承诺和结算奖励。

## 项目简介

Mindmark 同时处理两个相互连接的问题：如何让 AI 生成的内容真正成为可学习、可复习的材料，以及如何让 AI 代理的工作量对应到可预先约束、事后可核验的支付。系统先把资料整理成有顺序的章节，再把每章划分为几个独立的内容片段，作为不同的生成任务。AI 针对每个任务生成知识卡片候选，质量检查会核对卡片是否引用原文、难度是否合适、是否覆盖章节重点；每个任务的预算在生成前就按预估工作量锁定，只有质量检查通过并完成对应的链上结果登记后，才会向工作代理释放奖励。系统不把所有 AI 工作合并为一个不可拆分的总结果，而是让每个 Work Unit 分别形成 Worker 身份、输入哈希、卡片结果根和冻结报价的对应关系，从而支持逐项核验、逐项结算和失败隔离。

作为 Monad Hackathon 项目，Mindmark 将 Monad 用作这条支付链路的执行层：利用 EVM 兼容性复用 Solidity、Foundry 和现有钱包，利用低成本、高吞吐和较快确认支持细粒度工作结算。随着多个项目和 Worker 并行运行，Project 登记、Work Unit 承诺、Chapter 最终根和 Reward 释放会持续产生彼此独立的合约调用；Monad 让系统能够保留每项任务的独立证据和结算记录，而不必为了减少交易数量把它们聚合成一笔难以分别核验的操作。这些交易由固定的工作流和合约方法触发，不是由大模型自由构造。PDF、卡片正文和 FSRS 复习调度算法的明文状态仍保存在链下，Monad 只处理需要共享约束的 Project 身份、哈希承诺、预算和结算状态。
