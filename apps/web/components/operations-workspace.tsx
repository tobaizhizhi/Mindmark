...
import type { WorkflowOperationsSnapshot } from "@mindmark/shared";
...
async function parseApi(response: Response): Promise<WorkflowOperationsSnapshot> {
  const body = await response.json().catch(() => ({})) as WorkflowOperationsSnapshot & ApiError;
  if (!response.ok) {
...
export function OperationsWorkspace() {
  ...
  const refresh = useCallback(async (manual = false) => {
    ...
    try {
      const next = await parseApi(await fetch("/api/operations", { cache: "no-store" }));
      setSnapshot(next);
      setError(null);
    } catch (caught) {
